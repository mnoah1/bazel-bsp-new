[![Bazel Steward badge](https://img.shields.io/badge/Bazel_Steward-helping-blue.svg?style=flat&logo=data:image/png;base64,)](https://github.com/VirtusLab/bazel-steward)

# Bazel BSP (Build Server Protocol)

An implementation of the Build Server Protocol for Bazel, supporting languages such as Java, Kotlin, Scala, Python, Thrift, and Rust.

Initially as part of the [Hirschgarten project](https://github.com/JetBrains/hirschgarten), it has been split into a standalone project.

## Contributing

We welcome contributions to Bazel BSP!  
Please refer to our [Contribution Guidelines](CONTRIBUTING.md) for more information on how to get involved.

## License

Bazel BSP is licensed under the Apache License, Version 2.0. See the [LICENSE](LICENSE) file for details.

---

For more detailed information about each component, please refer to their respective directories within this repository.
